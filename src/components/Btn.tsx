  ...
  return (
    ...
    >
      ...
      <span className="flex gap-1">
        {IconTextStart && (
          <IconTextStart size={20} weight={iconWeight} className="mx-2" />
        )}
      </span>
      <span className="flex gap-1">
        {title}
        {IconTextEnd && (
          <IconTextEnd size={20} weight={iconWeight} className="mx-2" />
        )}
      </span>
...
function getBtnStyles(
  ...
  let finalClassName = "";
  const baseStyle =
    "flex py-2 px-4 items-center justify-center rounded-full w-full m-h-12 font-bold cursor-pointer ";
  ...
  switch (color) {
    case "brand":
      if (style === "filled")
        finalClassName += " bg-orange-500 text-stone-100 ";
      if (style === "light")
        finalClassName += " bg-orange-100 text-orange-600 ";
      break;
    case "danger":
